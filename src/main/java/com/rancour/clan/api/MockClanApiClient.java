...
public final class MockClanApiClient implements ClanApiClient
{
	...
	@Override public CompletionStage<Team> editTeam(String id, TeamEditRequest request, String token) { return done(new Team(id, request.getActivity(), "Mock Host", Collections.emptyList(), 1, request.getCapacity(), request.getWorld(), Boolean.TRUE.equals(request.getVoiceRequired()), "open", false, request.getTags(), true, Collections.singletonList("Mock Host"), Instant.now().toString(), Instant.now().plusSeconds(7200).toString(), null, null, request.getNotes())); }
